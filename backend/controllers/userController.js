...
class UserController {
  async createUser(req, res) {
    ...
    try {
        ...
        const user = await UserService.createUser(username, email, password, userType);
        logger.info(`User created successfully: ${JSON.stringify(user)}`);
        res.status(201).json({ message: "User created successfully", user });
    } catch (error) {
        logger.error(`User creation failed due to server error: ${error.message}`);
  ...
  async getUserById(req, res) {
    ...
    try {
      ...
      res.status(500).json({ error: "Failed to fetch user due to server error." });
    }
  }
  ...
  async getAllUsers(req, res) {
    try {
      ...
      logger.error(`Failed to fetch users due to server error: ${error.message}`);
